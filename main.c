...
int elaborate_packet(const u_char * Buffer, int Size)
{
    ...
    /*!(strstr(source_addr, "172.16") && */
    if (!strstr(source_addr, "10.100")) {
...
void elaborate_and_print_ip_header(const u_char * Buffer, int Size)
{

    if (elaborate_packet(Buffer, Size) == 1) {
        return;
    }
